...
public final class JSurbtc {
    ...
    public JSurbtc(final String key, final String secret, final LongSupplier nonceSupplier, final HttpHost proxy, final JSON json) {
        this(HTTPClient.newInstance(key, secret, nonceSupplier, proxy), newBigDecimalFormat(), json);
    }
    ...
    public Optional<ApiKey> newAPIKey(final String name, final Instant expiration) {
    ...
    public Optional<Order> newOrder(final MarketID marketId, final OrderType orderType, final OrderPriceType orderPriceType, final BigDecimal qty, final BigDecimal price) {
    ...
    public Optional<Order> cancelOrder(final long orderId) {
    ...
    public List<Market> getMarkets() {
        ...
        return get(path, MarketsDTO.class, MarketsDTO::getMarkets).orElse(emptyList());
    }

    public Optional<Ticker> getTicker(final MarketID marketId) {
        final String path = format("/api/v2/markets/%s/ticker", marketId).toLowerCase();
        return get(path, TickerDTO.class, TickerDTO::getTicker);
    }

    public Optional<OrderBook> getOrderBook(final MarketID marketId) {
    ...
    public List<Balance> getBalances() {
        return get("/api/v2/balances", BalancesDTO.class, BalancesDTO::getBalances).orElse(emptyList());
    }

    public List<Order> getOrders(final MarketID marketId) {
        final String path = format("/api/v2/markets/%s/orders", marketId).toLowerCase();
        return newPaginatedList(path, OrdersDTO.class, OrdersDTO::getPagination, OrdersDTO::getOrders);
    }

    public List<Order> getOrders(final MarketID marketId, final OrderState orderState) {
        final String path = format("/api/v2/markets/%s/orders?state=%s&algo=", marketId, orderState).toLowerCase();
        return newPaginatedList(path, OrdersDTO.class, OrdersDTO::getPagination, OrdersDTO::getOrders);
    }

    public List<Order> getOrders(final MarketID marketId, final BigDecimal minimunExchanged) {
    ...
    public List<Order> getOrders(final MarketID marketId, final OrderState orderState, final BigDecimal minimunExchanged) {
    ...
    public List<Deposit> getDeposits(final Currency currency) {
    ...
    public List<Withdrawal> getWithdrawals(final Currency currency) {
    ...
    public List<BalanceEvent> getBalanceEvents(final Currency currency) {
        ...
        return get(path + params, BalanceEventsDTO.class, BalanceEventsDTO::getBalanceEvents).orElse(emptyList());
    }
    ...
    private <T, K> JSurbtcPaginatedList<T> newPaginatedList(final String path,
                                                            ...
                                                            final Function<K, List<T>> getPage) {
        final IntFunction<List<T>> nextPageFunction = getNextPageFunction(path, dtoType, getPage);

        return get(path, (__, responseBody) -> {
            final Optional<K> dto = json.parse(responseBody, dtoType);

            final PaginationDTO pagination = dto.map(getPagination).orElse(null);
            if (pagination == null) {
                return Optional.empty();
            }

            final List<T> page = dto.map(getPage).orElse(emptyList());

            final JSurbtcPaginatedList<T> ts = new JSurbtcPaginatedList<>(page, nextPageFunction, pagination.getTotalPages(), pagination.getTotalCount());

            return Optional.of(ts);
        }).orElse(JSurbtcPaginatedList.empty());
    }
    ...
    private <K, T> IntFunction<List<T>> getNextPageFunction(final String path, final Class<K> dtoType, final Function<K, List<T>> getPage) {
        return page -> {
            ...
            return get(nextPath, dtoType, getPage).orElse(emptyList());
        };
    }
    ...
    private <T, K> Optional<K> get(final String path, final Class<T> valueType, final Function<T, K> mapper) {
        return get(path, (__, responseBody) -> json.parse(responseBody, valueType).map(mapper));
    }

    private <T, K> Optional<K> get(final String path, final HTTPClient.HTTPResponseHandler<K> responseHandler) {
        return httpClient.get(path, handlingErrors(responseHandler));
    }

    private <T> Optional<T> post(final String path, final Object payload, final HTTPClient.HTTPResponseHandler<T> responseHandler) {
    ...
    private <T, K> HTTPClient.HTTPResponseHandler<K> parser(final Class<T> valueType, final Function<T, K> mapper) {
        return (statusCode, responseBody) -> json.parse(responseBody, valueType).map(mapper);
    }
    ...
    private <T> HTTPClient.HTTPResponseHandler<T> handlingErrors(final HTTPClient.HTTPResponseHandler<T> responseHandler) {
        return (statusCode, responseBody) -> {
            ...
            if (!successful) {
                final ExceptionDTO exceptionDTO = json.parse(responseBody, ExceptionDTO.class).orElse(null);
